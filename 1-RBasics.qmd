...
```{r}
paste("Hello, everyone!", "R is a great language for historians!")
```

(@) Can you explain what happened in 2-3 sentences? 

> Paste is a way to connect strings. I was able to connect two strings, or sentences, by using the paste function.
...
> These codes are comparing numbers to one another. For example, 3 < 4 will return a result of TRUE because three is indeed less than four. To include another example, 5 != 5 will return a result of FALSE, because 5 is equal to 5 (not an inequality). 
...
```{r}
this.is.a.variable -> "Did I assign it a string? lol"
```
...
```{r}
number.of.tax.records <- 10
number.of.citizens <- 100
number.of.citizens*number.of.tax.records

```

(@) Try creating two variables that store strings. Can you concatenate those two variables together? 
```{r}
tax <- "Tax records for citizen living in 1803."
deed <- "Deed records for citizens living in 1803."
paste("Deed records for citizens living in 1803.","Tax records for citizen living in 1803.")
...
> The code multiplied each number in the vector by two. Therefore, the result is a list of numbers (2 times 2 equals 4, 2 times 4 equals 8, etc.)
...
> This time, the code multiplied 2 by the third number in the vector. Therefore, the result is 12, because the third number in the vector is 6, and 6 times 2 is 12.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
tax.records <- c(2,4,6,8,10)
```
...
> It looks like z is an equation, so it is solving the equation using the numbers in each row (x and y).
...
> From the description, this dataset contains information transcribed from Bob Damron's Address Books from 1965 through 1985.
...
> Honestly, I just googled these questions. But now it makes sense, heads and tails!
...
> This code means we want a segment of data in the gayguides dataset. The segment we want is the title of the 500th row in the dataframe.
...
> I opened the State Populations dataset, and tried to find the smallest population for a particular year, largest population for a particular year, and the average state population for a particular year, but it kept returning NA as an output. I Googled this, and I read that getting NA as an output means that there is missing data. This is because different states joined in different years. There really isn't "missing" data, in this case. I Googled again, and found that I could do something like: mean(statepopulations$X1820,na.rm+TRUE) to find that the average state population in 1820 was around 357000 people. This function tells R to ignore the cells that contain NA instead of a number. Cool!
