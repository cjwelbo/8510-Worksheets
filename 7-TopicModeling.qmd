...
If you run `class(mb.dtm)` in your console you will notice that it now has a class of "DocumentTermMatrix".Typo? I ran class(ac.dtm)
...
```{r}
ac.lda <- LDA(ac.dtm, k = 2, control = list(seed = 12345), method="Gibbs", alpha = 0.5)
ac.lda
```
...
ac.top.terms.test %>%
  ...
  scale_y_reordered()
```
I retrained the model on 2 topics, but it would not let me do just one topic. I changed the code to use Gibbs sampling, but I still got the error message when trying to return just one topic.
...
I still cannot get the section below to work. Was it something to do with file names that repeat with the BB dataset? I seem to remember other students having trouble, and it was something to do with the BB file names...
...
```{r}
meta.mind.body <- as.data.frame(mb.metadata)
mb.metadata$Filename <- paste("MB_", meta$Filename, sep="")
file_paths <- system.file("mbtxt")
mb_texts <- readtext(paste("mbtxt","*.txt", sep="")) #error: Error in list_files(file, ignore_missing, TRUE, verbosity) : File '' does not exist.
...
```{r}
tidy_bb_words <- bb.joined.rel.freq %>% count(Filename, word)
```
```{r}
bb.dtm <- tidy_bb_words %>% count (Filename, word) %>% cast_dtm (Filename, word, n)
```
```{r}
bb.lda <- LDA(bb.dtm, k = 15, control = list(seed=12345), method = "Gibbs", alpha = 0.5)
bb.lda
```
```{r}
...
wordcloud(topic1$term, topic1$beta, max.words = 50, random.order = FALSE, rot.per = 0.3, colors = brewer.pal(6, "Dark2"))
```
```{r}
bb.documents <- tidy(bb.lda, matrix = "gamma")
head(bb.documents)
```
```{r}
bb.documents %>% filter(document == "MB_1934_Feb.txt") %>% arrange(desc(gamma))
```
```{r}
topics.by.year <- full_join (bb.documents, bb.metadata.date.separated, by = join_by(document == Filename))
```
```{r}
topics.by.chronyear <- paste(topics.by.year$month, " ", 
                             topics.by.year$year)
ggplot(data=topics.by.year, aes(x = topics.by.chronyear, y = gamma)) +
  geom_bar(stat = "identity") +
  facet_wrap(~topic.scales = "free") +
  theme(axis.text.x = element_text(angle =90, vjust = 0.5, hjust = 1)) #I am lost here, I get an error: Error: unexpected '=' in:"  geom_bar(stat = "identity") + facet_wrap(~topic.scales ="
```


(@) Finally, when you are all done. Write up your findings. What research question did you focus on and what did you learn? 

This is not a finished worksheet, but I wanted to provide an update and ask for some help moving forward. I am able to get some code running for the BB dataset, and I even trained one model and played around with the number of topics, but I am not sure how to move from that to being able to actually answer the historical questions posed in the worksheet...
